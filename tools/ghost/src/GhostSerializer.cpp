...
void GhostSerializer::SetJsonLoaderCallback(JsonLoaderCallback callback)
{
	s_jsonLoaderCallback = callback;
}

ghost_json GhostSerializer::ReadJsonFromFile(const std::string& filename)
{
	// If a custom loader callback is registered, use it
	if (s_jsonLoaderCallback)
	{
		return s_jsonLoaderCallback(filename);
	}

	// Otherwise, fall back to direct file loading
	try
	{
		...
		else if (type == "radiobutton")
	{
		...
		else if (type == "iconbutton")
		{
			...
			if (!spriteDef.spritesheet.empty())
			{
				...
				Texture loadedTexture = LoadTextureWithEmbeddedSupport(spriteDef.spritesheet);
...
Font* GhostSerializer::LoadFontWithInheritance(const ghost_json& element, const ghost_json& inheritedProps, int elementID, std::string& outFontName, int& outFontSize)
{
	...
	if (!outFontName.empty())
	{
		...
		// Try to load from GhostResourceLoader (supports embedded resources)
		std::vector<unsigned char> fontData = GhostResourceLoader::LoadBinaryFile("Fonts/" + outFontName);

		if (!fontData.empty())
		{
			// Load font from memory
			const char* fileExt = GetFileExtension(outFontName.c_str());
			font = LoadFontFromMemory(fileExt, fontData.data(), fontData.size(), outFontSize, 0, 0);

			if (font.texture.id == 0)
			{
				Log("GhostSerializer::LoadFontWithInheritance - Failed to load font from memory: " + fontPath + ", using default");
				font = GetFontDefault();
			}
			else
			{
				Log("GhostSerializer::LoadFontWithInheritance - Loaded font '" + fontPath + "' from embedded resources at size " + std::to_string(outFontSize));
			}
		}
		else
		{
...
Texture GhostSerializer::LoadTextureWithEmbeddedSupport(const std::string& relativePath)
{
	std::string fullPath = s_baseSpritePath + relativePath;

	// Try to load from GhostResourceLoader (supports embedded resources)
	std::vector<unsigned char> imageData = GhostResourceLoader::LoadBinaryFile(relativePath);

	if (!imageData.empty())
	{
		// Load image from memory
		Image img = LoadImageFromMemory(GetFileExtension(relativePath.c_str()),
		                                 imageData.data(),
		                                 imageData.size());
		if (img.data != nullptr)
		{
			...
			if (tex.id != 0)
			{
				Log("GhostSerializer::LoadTextureWithEmbeddedSupport - Loaded from embedded resources: " + fullPath);
				return tex;
			}
		}
	}
	...
	if (tex.id != 0)
	{
		Log("GhostSerializer::LoadTextureWithEmbeddedSupport - Loaded from file: " + fullPath);
	}
	else
	{
		Log("GhostSerializer::LoadTextureWithEmbeddedSupport - Failed to load: " + fullPath);
	}
	return tex;
}
