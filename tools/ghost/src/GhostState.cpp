...
void GhostState::Init(const std::string& configfile)
{
	...
	// Load config from embedded resources or file
	Log("Loading ghost.cfg");
	std::string configContent = GhostResourceLoader::LoadGhostConfig();

	// Try to get paths from config, or use defaults if config fails
	std::string ghostPath;

	if (configContent.empty())
	{
		Log("ERROR: Failed to load ghost.cfg! Using default paths.");
	...
	{
		// Parse config using Config API
		Config config;
		if (!config.ParseFromString(configContent))
		{
			Log("ERROR: Failed to parse ghost.cfg! Using default paths.");
			m_fontPath = "Data/Fonts/";
			m_spritePath = "Data/";
			ghostPath = "Gui/Ghost/";
		}
		else
		{
			...
			ghostPath = config.GetString("GhostPath");
		}
	}
	...
	// Register GhostResourceLoader callback for loading .ghost files (supports embedded resources)
	GhostSerializer::SetJsonLoaderCallback([](const std::string& filename) -> ghost_json {
		return GhostResourceLoader::LoadGhostJson(filename);
	});
	...
	// Load GUI from JSON (embedded resources or file)
	ghost_json json = GhostResourceLoader::LoadGhostJson(GhostSerializer::GetBaseGhostPath() + "ghost_app.ghost");
	if (!json.empty() && m_serializer->ParseJson(json, m_gui.get()))
	{
		Log("Successfully loaded ghost_app.ghost");
	}
	else
	{
