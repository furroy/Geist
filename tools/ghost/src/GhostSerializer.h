...
public:
	...
	// Callback type for loading .ghost JSON files
	using JsonLoaderCallback = std::function<ghost_json(const std::string&)>;

	// Set a custom JSON loader callback (for embedded resources)
	static void SetJsonLoaderCallback(JsonLoaderCallback callback);
...
private:
	...
	// Helper function to load texture with embedded resource support
	// Tries to load from GhostResourceLoader first (embedded), then falls back to file
	// relativePath should be relative to the sprite base path (e.g., "GUI/guielements.png")
	Texture LoadTextureWithEmbeddedSupport(const std::string& relativePath);
	...
	// Static JSON loader callback (for embedded resources)
	static JsonLoaderCallback s_jsonLoaderCallback;
};
