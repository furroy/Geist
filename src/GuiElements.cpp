...
void GuiTextButton::Draw()
{
	...
	if (!m_Active)
	{
		...
		DrawRectangleRoundedLinesEx(Rectangle{ m_Gui->m_Pos.x + int(m_Pos.x), m_Gui->m_Pos.y + int(m_Pos.y),	m_Width, m_Height },
			0.5f,
         5,
         1,
         m_BorderColor);
	...
	else
	{
		...
		DrawRectangleRoundedLinesEx(Rectangle{ m_Gui->m_Pos.x + int(m_Pos.x), m_Gui->m_Pos.y + int(m_Pos.y),
			m_Width, m_Height },
